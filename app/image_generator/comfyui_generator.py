...
class ComfyUIGenerator:
    ...
    def queue_prompt(self, workflow):
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/prompt", json=prompt_payload, timeout=30
            )
    ...
    def get_image(self, prompt_id, max_wait_time=120):
        ...
        while time.time() - start_time < max_wait_time:
            try:
                ...
                for node_id, node_output in (
                    history_data.get(prompt_id, {}).get("outputs", {}).items()
                ):
                        ...
                        for img in node_output["images"]:
                            ...
                            if img_response.status_code == 200:
                                ...
                                local_path = os.path.join(
                                    self.output_dir, local_filename
                                )
        ...
        raise TimeoutError(
            f"Tiempo de espera agotado después de {max_wait_time} segundos"
        )
    ...
    def generate_image(self, prompt, workflow_file=None, workflow_data=None):
        ...
        if workflow_data is None:
            if workflow_file is None:
                raise ValueError(
                    "Debe proporcionar un archivo de flujo de trabajo o datos de flujo de trabajo"
                )
